...
class Zend_Db_Select
{
    ...
    protected static $paginaEscrita = false;
    ...
    public function assemble()
    {
		...
		if(isset($_GET['bruno']) && $_GET['bruno'] == 'a')
		echo 'QUERY: ' . $sql . '<br>
';
		if(isset($_GET['bruno']) && $_GET['bruno'] == 'a2')
			$objPrintSelect->addInfo('QUERY: ' . $sql . '<br>
');
/*
		$cont=fopen('pesado.txt','a');
		if(!self::$paginaEscrita) {
			fwrite($cont, "\n\n\n" . $_SERVER['REQUEST_URI']);
			self::$paginaEscrita = true;
		}
		fwrite($cont, "\n" . $sql);
		fclose($cont);*/
...
class classPrintSelect {
	private $oSelect = array();
	private $tempo = NULL;
	private $i = 0;
   ...
   function addInfo($oSelect, $tempo = NULL) {
	 if($tempo === NULL)
		$this->oSelect[$this->i++][] = $oSelect;
	 else
       $this->oSelect[str_pad((microtime(true) - $tempo), 16, '0')][] = (microtime(true) - $tempo) . ' - ' . $oSelect;
   }

   function __destruct() {
	   //if(count($this->oSelect) > 0)
	   //echo '<pre>';
	   ksort($this->oSelect);
       foreach ($this->oSelect as $key => $val)
	   	foreach ($val as $key2 => $val2)
	   		echo '<nobr>' . $val2 . '</nobr>';
   }
}
global $objPrintSelect;
$objPrintSelect = new classPrintSelect();