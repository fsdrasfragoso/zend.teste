...
class Cielo_Pedido
{
	...
	function request($paEndereco, $paPost){
		...
		curl_setopt($sessao_curl, CURLOPT_CAINFO, APPLICATION_PATH.'/ssl/VeriSignClass3PublicPrimaryCertificationAuthority-G5.crt');
		curl_setopt($sessao_curl, CURLOPT_SSLVERSION, 4);
		curl_setopt($sessao_curl, CURLOPT_CONNECTTIMEOUT, 10);
		curl_setopt($sessao_curl, CURLOPT_TIMEOUT, 9999);
	...
	public function Enviar($vmPost, $transacao)
	{
		...
		$vmResposta = $this->request(ENDERECO, "mensagem=" . $vmPost);
	...
	public function RequisicaoCancelamentoParcial()
	{
		$msg = $this->XMLHeader() . "\n" . 
			   '<requisicao-cancelamento id="' . md5(date("YmdHisu")) . '" versao="' . VERSAO2 . '">' . "\n   "
			    . '<tid>' . $this->tid . '</tid>' . "\n   "
			    . $this->XMLDadosEc() . "\n"
			    . '<valor>' . str_replace('.','', $this->valor) . '</valor>' . "\n   ".
			   '</requisicao-cancelamento>';
